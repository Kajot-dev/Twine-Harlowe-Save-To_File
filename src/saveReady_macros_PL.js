const privateSave = {
  ...
  verify: (text, filBool, commands, verSave) => {
    ...
    if ((text[0] !== "(" || text[6] !== " " || text[11] !== " ") || verSave) {
      if (commands) privateSave.openDialog("Nie można wczytać tego pliku - to nie jest zapis Twine!");
      return "corrupted_save_name";
    }
    if (StoryId !== privateSave.storyUID) {
      if (commands) privateSave.openDialog("Nie można wczytać tego zapisu - pochodzi z innej gry!");
      return "wrong_id";
    }
  ...
  process: (fileInput, targetslot, slots) => {
    return new Promise((resolve, reject) => {
      ...
      if (file.name.slice(file.name.lastIndexOf(".")+1).toLowerCase() !== privateSave.extension) {
        privateSave.openDialog("Nieprawidłowe rozszerzenie!");
        reject("wrong_extension");
        return;
      }
      if (!file || (file.size / 1024) > privateSave.maxFileSize) {
        privateSave.openDialog("Brak pliku/Zbyt duży plik!");
      ...
      fileRead.onload = ready => {
        ...
        if (input.length <= 50) {
          privateSave.openDialog("Plik pusty/zbyt krótki, jak na save!");
        ...
        if (input.slice(1, 11) !== "Saved Game") {
          privateSave.openDialog("Nie można wczytać tego zapisu - nie można go sprawdzić!");
          reject("no_phrase");
          return;
        }
        var saveArray = input.split('\n');
        if (saveArray.length < 2) {
          privateSave.openDialog("Nie można wczytać tego zapisu - nie da się go rozpakować!");
        ...
        if (slots.includes(saveName)) {
          if (!(State.deserialise(saveArray[1]) instanceof Error)) { //if this returns true, we are absolutely sure, that save is gonna work
            ...
            privateSave.openDialog("Nie można wczytać tego zapisu - jest nieprawidłowy!");
            reject("corrupted_save_data");
          }
        } else {
          privateSave.openDialog("Przepraszamy! Autor gry zablokował możliwość wczytania tego zapisu!");
          reject("wrong_slot");
        }
      };
      fileRead.onerror = () => {
        privateSave.openDialog("Błąd odczytu pliku!");
        reject("file_error");
      };
    });
  },
  file: (name, slot) => {
    ...
    if (saveOutput) {
      ...
      privateSave.openDialog("Nie można pobrać save'a który nie istnieje!");
      console.error(...privateSave.appPrefix, "Nie można pobrać save'a który nie istnieje!");
    }
  },
  fileDirect: (name, slot) => {
    let saveData = privateSave.getDirect(slot);
    if (typeof saveData === "null") {
      privateSave.openDialog("Nie udało się pobrać save'a - prawdopodobnie błąd Twine!");
      return null;
    }
  ...
  read: (slot, slots) => {
    ...
    fileSave.addEventListener("change", () => {
      privateSave.process(fileSave, slot, slots).then(() => {
        ...
        console.log(...privateSave.appPrefix, "Wczytywanie powiodło się!");
      }).catch(reason => {
        fileSave.remove();
        console.error(...privateSave.appPrefix, "Nie udało się wczytać z pliku, z powodu:", reason);
        if (reason == "cheater") {
...
console.info(privateSave.appPrefix[0]+"%c Zapis do pliku gotowy!", privateSave.appPrefix[1], "color: green;");
